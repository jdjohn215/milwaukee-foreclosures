...
foreclosure.rates.by.ald <- parcels.by.ald %>%
  ...
  select(start_year, aldermanic_2022, privately_owned_parcels) %>%
  inner_join(foreclosures.by.ald, by = c("start_year" = "foreclose_year", 
                                         "aldermanic_2022" = "aldermanic_2022")) %>%
  mutate(foreclosures_per_1k = (foreclosures/privately_owned_parcels)*1000)
...
foreclosure.rates.by.tract2020 <- parcels.by.tract2020 %>%
  mutate(start_year = year_end + 1) %>%
  select(start_year, tract_2020, privately_owned_parcels) %>%
...
foreclosure.rates.by.tract2010 <- parcels.by.tract2010 %>%
  mutate(start_year = year_end + 1) %>%
  select(start_year, tract_2010, privately_owned_parcels) %>%
...
foreclosure.rates.by.neighborhood <- parcels.by.neighborhood %>%
  mutate(start_year = year_end + 1) %>%
  select(start_year, neighborhood, privately_owned_parcels) %>%
  left_join(foreclosures.by.neighborhood, by = c("start_year" = "foreclose_year", 
                                               "neighborhood" = "neighborhood")) %>%
  mutate(foreclosures = if_else(is.na(foreclosures), 0, as.numeric(foreclosures)),
         foreclosures_per_1k = (foreclosures/privately_owned_parcels)*1000)
